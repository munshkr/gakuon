...
const noteCommands = [
  'I_NOTE_C', 'I_NOTE_CS', 'I_NOTE_D', 'I_NOTE_DS',
  'I_NOTE_E', 'I_NOTE_F', 'I_NOTE_FS', 'I_NOTE_G',
  'I_NOTE_GS', 'I_NOTE_A', 'I_NOTE_AS', 'I_NOTE_B'
];
      ...
      switch (param) {
        case 'adsr': {
          if (value.type === 'sequence' && value.value.length === 4) {
            ...
            song.push('I_ADSR', (seq[1]<<3)|seq[0], (seq[3]<<3)|seq[2]);
          } else {
    ...
    for (let cmd of sequence) {
      ...
      switch (cmd.type) {
        ...
        case 'set_octave':
          ...
          if (!updateOnlyState) {
            song.push('I_OCTAVE', state.octave);
          }
          break;
        case 'inc_octave':
          state.octave += 1;
        ...
        case 'set_volume':
          ...
          if (!updateOnlyState) {
            song.push('I_VOLUME', state.volume);
          }
          break;
        case 'inc_volume':
          if (cmd.value) {
