...
class CodeGenerator {
  ...
  generate(document) {
    return this._compileAST(document);
  }

  _compileAST(document) {
    let data = { debug: this.options.debug };

    data.headers = this._buildHeaders(document);

    let initialStates = this._buildInitialState(document.AST);
    data.initialStates = initialStates;

    data.sequences = this._buildSequences(document.AST);
    data.song = this._buildSongTables(document.AST, initialStates);

    return this._template(data);
  }

  _buildHeaders(document) {
    let headers = {};

    for (let dir of document.directives) {
