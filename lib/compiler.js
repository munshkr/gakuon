...
class TemplateHelpers {
  ...
  static byteTable(values) {
    let ss = '';
    values.eachSlice(10, (slice) => {
      ss = ss + '  .byte ' + slice.map((v) => { return this.asm(v); }).join(', ') + '\n';
    });
    return ss;
  }
};
...
function compileAST(ast, options) {
  options = Object.assign(options || {}, {
    filename: path.join(__dirname, '../lib/player/tune'),
    context: TemplateHelpers
  });
  ...
  var template = ejs.compile(options.template, options);
